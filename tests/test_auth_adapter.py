...
VoiceVerifier is mocked so these tests only verify that
VoiceAuthenticator correctly converts verification results
into AuthenticationResult objects.
"""

import pytest

import vocalid.auth_adapter as auth_module
from vocalid.auth_adapter import (
    VoiceAuthenticator,
...
def test_authenticate_live_returns_denied_result(
    authenticator,
    monkeypatch
):

    monkeypatch.setattr(
        "vocalid.recorder.record_one",
        lambda seconds, sample_rate: "fake_audio"
    )

    result = authenticator.authenticate_live(
        seconds=4.0
    )
...
def test_authenticate_live_defaults_to_four_seconds(
    authenticator,
    monkeypatch
):
    ...
    authenticator.authenticate_live()
